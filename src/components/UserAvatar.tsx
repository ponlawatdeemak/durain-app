...
export default function UserAvatar(props: Props) {
    ...
    const sizeClass = (size == "large") ? {fontSize: 48, color: 'white', width: 96, height: 96, m: 0} 
        : (size=="small") ? {fontSize: 14, color: 'white', width: 24, height: 24, m: 0} 
        : {fontSize: 20, color: 'white', width: 32, height: 32, m: 0}

    return <>
        { !user && <ProfileIcon sx={sizeClass} /> }
        { user?.image ? <Avatar src={user?.image} alt={firstName} sx={sizeClass} /> 
            : user ? <Avatar alt={firstName} sx={sizeClass}>{avatarTextName}</Avatar> : <></>
        }
    </>
}
